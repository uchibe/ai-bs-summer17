...
class Circuit2cTurtlebotCameraNnEnv(gym.Env):
    ...
    def _step(self, action):
        ...
        #while depth_data is None or success_depth is False:
        #    try:
        #        depth_data=rospy.wait_for_message('/camera/depth/image_raw',Image,)
        ...
        while image_data is None or success is False:
            try:
                ...
                #np.save('img.npy',image_data)
                h = image_data.height
                w = image_data.width
                cv_image = CvBridge().imgmsg_to_cv2(image_data, "bgr8")
                #cv_depth = CvBridge().imgmsg_to_cv2(depth_data, "passthrough")
                #np.save('cvdepth.npy',cv_depth)
                #np.save('cvimg.npy',cv_image)
        ...
        while depth_data is None or success_depth is False:
            try:
                depth_data=rospy.wait_for_message('/camera/depth/image_raw',Image,timeout=5)
                ht=depth_data.height
                wt=depth_data.width
                #print(ht)
                #print(wt)
                cv_depth=CvBridge().imgmsg_to_cv2(depth_data,"passthrough")
                #print(cv_depth.shape)
                #print(cv_depth[ht/2,wt/2])
                if not (cv_depth[ht/2,wt/2]==178):
                    success_depth=True
                else:
                    pass
                #print(success_depth)
        ...
        #np.save('grayimg.npy',cv_image)
        cv_image = cv2.resize(cv_image, (self.img_rows, self.img_cols))
        #np.save('resizegrayimg.npy',cv_image)
